...
class DBHelper{
  ...
  _createDatabase(Database db, int version) async{
    await db.execute(
      "CREATE TABLE myfolder(id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL)",
    );
    await db.execute(
      "CREATE TABLE myfile(id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, doc_path STRING, folder_id INTEGER)",
    );
  }
...
//Folder methods
  ...
  Future<List<DocModel>> getDoc(int? file_id) async{
    await db;
    final List<Map<String, Object?>> QueryResult = await _db!.rawQuery("SELECT * FROM myfile WHERE ',' || doc_path || ',' LIKE '%,' || ? || ',%'", ["$file_id"]);
